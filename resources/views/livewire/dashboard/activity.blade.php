      ...
      <div class="div" wire:key="{{ $activity->id }}">
        <div class="col d-flex justify-content-center">
            <div class="card" style="width: 18rem;">
              <img src="{{ asset('storage/aktivitas/'.$activity->foto_aktivitas) }}" class="card-img-top img-card" alt="...">
              <div class="card-body text-center">
                <a href="#" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#modalHapusGambar"
                wire:click="openModal({{ $activity->id }})">Hapus Foto</a>
                <a href="#" class="btn btn-primary ms-4" data-bs-toggle="modal" data-bs-target="#modalEditGambar" 
                wire:click="openModal({{ $activity->id }})">Ganti Foto</a>
              </div>
          </div>
      </div>
      </div>
      ...
      <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
          ...
          @if ($image)
            <img src="{{ $image->temporaryUrl() }}" class="img-thumbnail img-preview" alt="preview">
          @elseif ($idImage)
            <img src="{{ asset('storage/aktivitas/'.$idImage->foto_aktivitas) }}" class="img-thumbnail img-preview" alt="preview">
          @else 
            <img src="{{ asset('images/dashboard/blankImage.jpg') }}" class="img-thumbnail img-preview" alt="preview">
          @endif
          @if ($idImage)
