            ...
            <div class="div" wire:key="{{ $galeri->id }}">
                <div class="col d-flex justify-content-center">
                    <div class="card" style="width: 18rem;">
                        <img src="{{ asset('storage/galeri/' . $galeri->foto_galeri) }}"
                            class="card-img-top img-card" alt="...">
                        <div class="card-body text-center">
                            <a href="#" class="btn btn-danger" data-bs-toggle="modal"
                                data-bs-target="#modalHapusGambar" wire:click="openModal({{ $galeri->id }})">Hapus
                                Foto</a>
                            <a href="#" class="btn btn-primary ms-4" data-bs-toggle="modal"
                                data-bs-target="#modalEditGambar" wire:click="openModal({{ $galeri->id }})">Ganti
                                Foto</a>
                        </div>
                    </div>
                </div>
    ...
    @else
        <div class="ppdb-closed-container container text-center">
            <div class="ppdb-closed-content">
                <h3 class="ppdb-title">Belum ada Galeri</h3>
                <p class="ppdb-message">Silahkan Menambah Galeri untuk di tampilkan di Company Profile </p>
            </div>
        </div>
    @endif
</div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    @if ($image)
                        <img src="{{ $image->temporaryUrl() }}" class="img-thumbnail img-preview" alt="preview">
                    @elseif ($idImage)
                        <img src="{{ asset('storage/galeri/' . $idImage->foto_galeri) }}"
                    ...
                    @endif
                    @if ($idImage)
                        <form wire:submit="update({{ $idImage->id }})" id="form-edit"
